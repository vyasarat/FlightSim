...
await page.evaluate(()=>renderer.render(scene,camera));await page.screenshot({path:path.join(out,'eject-mars-arrival.png')});
async function cycle(name){const progress=await page.evaluate(()=>JSON.stringify({rocks:rover.rocks.map(r=>r.lit),beacons:rover.beacons.length}));await tap('#ejectBtn');let impact,landShot;for(let i=0;i<400;i++){await step(.05);if(await page.evaluate(()=>eject.phase==='float')&&!impact){await page.evaluate(()=>renderer.render(scene,camera));await page.screenshot({path:path.join(out,`eject-${name}-float.png`)});impact=true;}if(!landShot&&await page.evaluate(()=>eject.phase==='land')){await page.evaluate(()=>renderer.render(scene,camera));await page.screenshot({path:path.join(out,`eject-${name}-land.png`)});landShot=true;}if(await page.evaluate(()=>!eject.active))break;}const result=await page.evaluate(()=>({last:eject.last,body:rk.onBody?.name,rover:roverActive(),drone:marsDroneActive(),active:eject.active}));const after=await page.evaluate(()=>JSON.stringify({rocks:rover.rocks.map(r=>r.lit),beacons:rover.beacons.length}));if(after!==progress)throw Error(name+' lost surface progress');console.log(name,JSON.stringify(result));if(!landShot)throw Error(name+' missed visible landing');if(result.last?.family!==(name==='mars-rover'?'rover':name==='mars-drone'?'drone':'rocket'))throw Error(name+' wrong family');if(!result.last?.returned||result.active||!result.last.canopyAtImpact||result.last.contactError>.02)throw Error(name+' failed');}
await cycle('mars-rocket');await step(1);await tap('#hopBtn');await step(1);await cycle('mars-rover');await step(1);
...
await tap('#hopBtn');await step(1);if(!await page.evaluate(()=>marsDroneActive()))throw Error('Drone entry failed');await cycle('mars-drone');
