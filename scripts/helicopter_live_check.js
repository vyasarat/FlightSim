...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>typeof toyHelicopter==='function'&&!document.getElementById('heliHoverBtn'),{},{timeout:120000,polling:100});
    await page.waitForFunction(()=>window.__lp);
    const upgraded=await page.evaluate(async()=>({hoverButton:!!document.getElementById('heliHoverBtn'),altitudeColor:getComputedStyle(el.heliUpBtn).color,caches:await caches.keys(),vehicle:state.vehicleKey,phase:state.phase,yards:toyWorld.yards.length,greeting:TW.greeting.duration}));
    if(upgraded.hoverButton||upgraded.altitudeColor!=='rgb(242, 244, 247)'||upgraded.phase!=='TAXI'||upgraded.vehicle!=='helicopter'||!upgraded.caches.includes('little-pilot-cockpit-v87-toyfinish')||upgraded.caches.some(k=>k.startsWith('little-pilot-cockpit-')&&k!=='little-pilot-cockpit-v87-toyfinish'))throw Error(JSON.stringify(upgraded));
    ...
    await page.screenshot({path:path.resolve(__dirname,'../qa-screenshots/live-v87-offline-pinwheel.png')});
    ...
    await page.screenshot({path:path.resolve(__dirname,'../qa-screenshots/live-v87-offline-eject.png')});
