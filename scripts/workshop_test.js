...
(async () => {
  ...
  try {
    ...
    await require(process.argv.includes('--behavior')?'./workshop_checks':'./workshop_play_checks')({ newPage, shots: path.resolve(__dirname, '../qa-screenshots'), check: (name, ok, details) => { results.push(!!ok); console.log(`${ok ? 'PASS' : 'FAIL'} ${name} ${details || ''}`); } });
