...
(async()=>{
 ...
 try{
  const results=[];
  for(const key of (process.argv.includes('--all')?['prop','helicopter','fighter','airlinerDelta','airlinerJetblue','airlinerEmirates']:['prop','helicopter'])){
   ...
   const shot=name=>page.screenshot({path:path.resolve(__dirname,`../qa-screenshots/eject-${key}-${process.argv.includes('--ipad')?'ipad-':''}${name}.png`)});
   ...
   await tap(`[data-v="${key}"]`);await tap('[data-d="0"]');await step(1);await shot('ready');
