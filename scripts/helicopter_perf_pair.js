...
(async()=>{
 ...
 for(const [width,height] of [[1024,768],[390,844]]){
  ...
  await both(async p=>{await p.step(12);await p.hold('#heliDownBtn',20);await p.step(1);await p.button('#vehBtn');await p.button('[data-v="helicopter"]');await p.button('[data-d="0"]');await p.step(2);await p.hold('#heliUpBtn',4);await p.step(1);await p.aim('wind');await p.step(12);});await measure('pinwheel');
  await both(p=>p.hold('#heliDownBtn',7.1));await measure('downwash');
  await both(async p=>{await p.button('#ejectBtn');await p.step(4.5);});await measure('rescue');
  await both(async p=>{await p.step(30);await p.page.close();});
 }
 }finally{fs.writeFileSync(path.resolve(__dirname,'../docs/helicopter-polish/performance-paired.json'),JSON.stringify({environment:{platform:process.platform,arch:process.arch,chrome:await browser.version(),baseline:'cb42b8b',method:'Alternating baseline/candidate order each sample. 4 warmups + 20 frames per scene. CPU includes update + submission; frame includes blocking pixel readback. Total calls/tris include shadow pass. SwiftShader, not physical iPad.'},rows},null,2));await browser.close();for(const server of servers)await new Promise(r=>server.close(r));}
