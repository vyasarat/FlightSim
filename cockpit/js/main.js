...
window.__lp = {
  ...
  EVENT_POOLS, evpRegister, evpPool, evpMember, evpAll, evpEventPools, evpDraw, evpDue, evpRearm, evpForce, SPACE_POOL, SEA_POOL,
  ...
  forEachSolid(cb){for(const b of buildingBoxes)cb(b);for(const b of staticSolids)cb(b);for(const arr of streamedSolids.values())for(const b of arr)cb(b);for(const b of trainSolids)cb(b);}, get vehicleModel(){return vehicleModel;}, vehPlayerModels, vehTagPlayer,
